...
function showAdminView(id){
  ...
  const map={
    ...
    '#viewInvoice':['Invoice design','Branding & layout',loadInvoice],
...
function savePrefs(){
  let s={};try{s=JSON.parse(localStorage.getItem('atom_prefs')||'{}')}catch(e){}
  s.store=($('#prefStore')?.value||'').trim();s.phone=($('#prefPhone')?.value||'').trim();s.addr=($('#prefAddr')?.value||'').trim();s.gst=+($('#prefGst')?.value||0);s.low=+($('#prefLow')?.value||5);
  localStorage.setItem('atom_prefs',JSON.stringify(s));
...
(async()=>{
  ...
  $$('.adm-sub button').forEach(b=>{
    b.onclick=()=>{
      ...
      else if(a==='invoice'){showAdminView('#viewInvoice');loadInvoice()}
  ...
  if($('#btnSaveInv'))$('#btnSaveInv').onclick=saveInvoice;
